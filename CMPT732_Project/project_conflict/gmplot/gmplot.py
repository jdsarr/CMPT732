...
class GoogleMapPlotter(object):
    ...
    def __init__(self, center_lat, center_lng, zoom, marker_path=None):
        ...
        self.coloricon = os.path.join(path, 'markers/%s.png')
    ...
    def marker(self, lat, lng, comment, color='#FF0000', c=None):
        if c:
            color = c
        color = self.color_dict.get(color, color)
        color = self.html_color_codes.get(color, color)
        self.points.append((lat, lng, comment, color[1:]))

    def scatter(self, lats, lngs,comments=None, color=None, size=None, marker=True, c=None, s=None, **kwargs):
        color = color or c
        ...
        params = []
        if comments == None:
            zipped = zip(lats,lngs)
            for x in zipped:
                params.append((x[0],x[1],'No comment'))
        else:
            params = zip(lats,lngs,comments)

        for lat, lng, comment in params:
            if marker:
                self.marker(lat, lng, comment, settings['color'])
            else:
    ...
    def write_point(self, f, lat, lon, comment, color):
